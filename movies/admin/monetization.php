        ...
        try {
            ...
            if ($withdrawal) {
                ...
                if ($stmt->rowCount() > 0) {
                    ...
                    // Process payment via MoMo API
                    $result = $momo->processWithdrawal($withdrawal_id);
                    
                    if ($result['success']) {
                        $success_message = "Withdrawal approved and processed successfully via MoMo API!";
                        $success_message .= " Payment of " . number_format($withdrawal['amount'], 2) . " RWF sent to " . ($withdrawal['recipient_phone'] ?? 'N/A') . " via " . ($withdrawal['payment_method'] ?? 'N/A') . ".";
                        $success_message .= " Transaction ID: " . $result['transactionId'];
                    } else {
                        $error_message = "Withdrawal approved but MoMo API processing failed!";
                        $error_message .= " Manual processing required. Error: " . $result['error'];
                        // Log the error for debugging
                        error_log("MoMo Withdrawal Processing Failed - Withdrawal ID: $withdrawal_id, Error: " . $result['error']);
                    }
                } else {
...
// Add a new section to check withdrawal status
if (isset($_POST['check_status'])) {
    ...
    if ($withdrawal_id > 0) {
        try {
            ...
            // Check withdrawal status via MoMo API
            $result = $momo->checkWithdrawalStatus($withdrawal_id);
            
            if ($result['success']) {
                $status_message = "Withdrawal status: " . ucfirst($result['status']);
                if ($result['financialTransactionId']) {
                    $status_message .= " | Financial Transaction ID: " . $result['financialTransactionId'];
                }
            } else {
                $error_message = "Error checking withdrawal status: " . $result['error'];
            }
        } catch (Exception $e) {
            $error_message = "Error checking withdrawal status: " . $e->getMessage();
        }
    } else {
        $error_message = "Invalid withdrawal request.";
    }
            ...
            <div class="col-md-3 col-lg-2 d-md-block sidebar bg-dark">
                <div class="position-sticky pt-3">
                    <ul class="nav flex-column">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" href="momo_test.php">
                                <i class="bi bi-coin me-2"></i>MoMo Test
                            </a>
                        </li>
                        <li class="nav-item">
            ...
            <div class="col-md-9 ms-sm-auto col-lg-10 px-md-4 main-content">
                ...
                <?php if (isset($error_message)): ?>
                    <div class="alert alert-danger">
                        <?= $error_message ?>
                        <?php if (strpos($error_message, 'invalid subscription key') !== false): ?>
                            <br><strong>Solution:</strong> Your MoMo API subscription key is invalid. 
                            Please update your credentials in <code>momo_config.php</code> with valid keys from the 
                            <a href="https://momodeveloper.mtn.com/" target="_blank">MTN MoMo Developer Portal</a>.
                            See <code>MOMO_API_FIX_INSTRUCTIONS.txt</code> for detailed steps.
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
                                ...
                                try {
                                    ...
                                    while ($withdrawal = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                        ...
                                        if ($withdrawal['status'] == 'pending') {
                                            ...
                                            echo '<form method="post" style="display:inline;">
                                                    <input type="hidden" name="withdrawal_id" value="'.$withdrawal['id'].'">
                                                    <button type="submit" name="check_status" class="btn btn-sm btn-info">Check Status</button>
                                                  </form>';
                                        } else {
                                            echo '-';
                                        }
